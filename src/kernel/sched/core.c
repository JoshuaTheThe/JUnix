...
static bool override_store =
    false; // cancel store to current proc, e.g. interrupt after exit()
...
task_t *task_create(proc_t *proc) {
  ...
  paging_map(proc->space, KERNEL_STACK_VIRT + PAGE_SIZE * proc->taskcount++,
             phys, PAGE_PRESENT | PAGE_WRITE);
...
void proc_kill(proc_t *proc) { proc->awaiting_destruction = true; }
...
void sched_next(void) {
  for (;;) {
    ...
    if (current_task->state == TASK_RUNNING &&
        !current_proc->awaiting_destruction)
      break;
  }
...
int proc_open_direct(proc_t *proc, vnode_t *node, int flags, int mode) {
  ...
  new : if (fd == proc->fd.capacity || !proc->fd.items) {

...
int vfs_lookup(const char *path, vnode_t **out) {
  ...
  while (*path) {
    ...
    while (child) {
      if (strlen(child->name) == strlen(segment) &&
          !strncmp(child->name, segment, 1024)) {
        current = child;
        break;
      }
      child = child->next;
...
int vfs_mount(const char *path, filesystem_t *fs, void *data) {
  ...
  not_optional(fs);
  vnode_t *mountpoint;
  if (vfs_lookup(path, &mountpoint) != 0)
    return -1;
  if (mountpoint->children)
    return -1;

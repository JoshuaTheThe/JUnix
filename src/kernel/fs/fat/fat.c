...
static uint32_t total_sectors(fat_bpb_t *bt) {
  return (bt->short_sector_count) ? bt->short_sector_count
                                  : bt->long_sector_count;
}

static uint32_t size(fat_bpb_t *bt) {
  return (bt->sectors_per_fat) ? bt->sectors_per_fat
                               : bt->fat32.sectors_per_fat;
}

static uint32_t root_dir_size(fat_bpb_t *bt) {
  return ((bt->root_dir_entries * sizeof(fat_dir_t)) + (bt->sector_size - 1)) /
         bt->sector_size;
}

static uint32_t first_data(fat_bpb_t *bt) {
  return (bt->reserved_sectors + (bt->number_of_fats * size(bt)) +
          root_dir_size(bt));
}

static uint32_t first_fat(fat_bpb_t *bt) { return (bt->reserved_sectors); }

static uint32_t data_cnt(fat_bpb_t *bt) {
  return total_sectors(bt) -
         (bt->reserved_sectors + (bt->number_of_fats * size(bt)) +
          root_dir_size(bt));
}
...
__attribute__((__used__)) static uint32_t first_root(fat_bpb_t *bt) {
...
__attribute__((__used__)) static uint16_t
next_cluster_16(fat_bpb_t *bt, uint16_t cluster, file_t *file) {
...
__attribute__((__used__)) static uint16_t
next_cluster_12(fat_bpb_t *bt, uint16_t cluster, file_t *file) {
...
static void set_cluster(fat_bpb_t *bt, uint32_t cluster, uint32_t value,
                        file_t *file) {
...
__attribute__((__used__)) static uint32_t allocate_cluster(fat_bpb_t *bt,
                                                           file_t *file) {
...
static fat_dir_t fetch_directory(fat_bpb_t *bt, uint32_t cluster, file_t *file,
                                 size_t entry) {
...
static int iterate_directory(fat_bpb_t *bt, uint32_t dir, file_t *file,
                             int (*callback)(uint32_t, fat_dir_t *, size_t)) {
...
static int read(file_t *file, void *buf, size_t n) {
  ...
  fat_dir_t dir = fetch_directory(&priv->bpb, location->cluster, priv->base,
                                  location->index);
  uint32_t first_cluster = ((uint32_t)dir.entry_first_cluster_high << 16) |
  ...
  uint32_t cluster_bytes = priv->bpb.sector_size * priv->bpb.cluster_size;
  ...
  while (n) {
    uint32_t first_sector = first_sector_for_cluster(&priv->bpb, cluster);
    uint32_t sector = first_sector + (cluster_offset / priv->bpb.sector_size);

    uint32_t offset = cluster_offset % priv->bpb.sector_size;
    uint8_t sector_buf[priv->bpb.sector_size];
    vfs_lseek(priv->base, sector * priv->bpb.sector_size, SEEK_SET);
    vfs_read(priv->base, sector_buf, sizeof(sector_buf));
...
static void write_directory(fat_bpb_t *bt, uint32_t cluster, file_t *file,
                            size_t entry, fat_dir_t *dir) {
  uint32_t first = first_sector_for_cluster(bt, cluster);
...
static int write(file_t *file, const void *buf, size_t n) {
   ...
   */
  if (first_cluster == 0) {
    first_cluster = allocate_cluster(&priv->bpb, priv->base);

    if (!first_cluster)
      return -1;

    dir.entry_first_cluster_high = first_cluster >> 16;

    dir.entry_first_cluster_low = first_cluster & 0xffff;
  }
  ...
  for (uint32_t i = 0; i < skip; i++) {
    uint32_t next = next_cluster(&priv->bpb, cluster, priv->base);

    if (next >= 0x0FFFFFF8) {
      next = allocate_cluster(&priv->bpb, priv->base);

      if (!next)
        return -1;

      set_cluster(&priv->bpb, cluster, next, priv->base);
    }
  ...
  while (written < n) {
    ...
    memcpy(sector_buf + offset, in + written, copy);

    vfs_lseek(priv->base, sector * priv->bpb.sector_size, SEEK_SET);

    vfs_write(priv->base, sector_buf, sizeof(sector_buf));

    written += copy;
    cluster_offset += copy;

    if (cluster_offset >= cluster_bytes && written < n) {
      uint32_t next = next_cluster(&priv->bpb, cluster, priv->base);
  ...
  write_directory(&priv->bpb, location->cluster, priv->base, location->index,
                  &dir);
...
static char tolower(char x) { return x >= 'A' && x <= 'Z' ? x | 0x20 : x; }

static bool is_dot_entry(fat_dir_t *dir) {
  return dir->name[0] == '.' && dir->name[1] == ' ' && dir->name[2] == ' ' &&
         dir->name[3] == ' ' && dir->name[4] == ' ' && dir->name[5] == ' ' &&
         dir->name[6] == ' ' && dir->name[7] == ' ';
}
...
static int add_file(uint32_t clus, fat_dir_t *dir, size_t index) {
  ...
  if (dir->flags & FAT_DIRECTORY) {
    ...
    current_parent = node;
    uint32_t cluster = ((uint32_t)dir->entry_first_cluster_high << 16) |
                       (uint32_t)dir->entry_first_cluster_low;
    res = iterate_directory(&_priv->bpb, cluster, _priv->base, add_file);
  }
...
static int mount(vnode_t *node, vnode_t *in) {
  ...
  node->priv = priv;
  if (priv->type != FAT_32) {
    LOG(" [krnl] error: fat of type %d provided, only supports fat32, dumping "
        "bt\r\n",
        priv->type);
    dump(&priv->bpb, 512);
    LOG("._0XEBXX90: [%x, %x, %x]\r\n", priv->bpb._0XEBXX90[0] & 255,
        priv->bpb._0XEBXX90[1] & 255, priv->bpb._0XEBXX90[2] & 255);
    LOG(".OEM: \"%s\"\r\n", priv->bpb.OEM);
  ...
  return iterate_directory(&priv->bpb, root_cluster(&priv->bpb), priv->base,
                           add_file);
}

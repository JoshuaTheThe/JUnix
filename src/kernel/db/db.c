...
static db_t *db_get(vnode_t *node) { return (db_t *)node->priv; }
...
tabl_id_t db_create_table(vnode_t *db, char *name) {
  ...
  db_table_t *tables =
      db_realloc(database->tables, sizeof(db_table_t) * (id + 1));
  ...
  table->name = db_add_string(db, strlen(name), name);
...
bool db_set_table_geometry(vnode_t *db, tabl_id_t id, size_t count,
  ...
  table->col_types = db_alloc(sizeof(db_type_t) * count);

  if (!table->col_types)
    return false;

  memcpy(table->col_types, geometry, sizeof(db_type_t) * count);
...
row_id_t db_insert_row(vnode_t *db, tabl_id_t id, size_t count,
  ...
  if (table->row_count >= table->row_capacity) {
    size_t new_capacity = table->row_capacity ? table->row_capacity * 2 : 8;

    db_cell_t *cells = db_realloc(
        table->cells, sizeof(db_cell_t) * new_capacity * table->col_count);
  ...
  memcpy(&table->cells[row * table->col_count], data,
...
bool db_fetch_row(vnode_t *db, tabl_id_t t_id, row_id_t r_id, size_t count,
                  db_cell_t data[static count]) {
  ...
  memcpy(data, &table->cells[r_id * table->col_count],
         sizeof(db_cell_t) * count);

  return true;
}

bool db_update_row(vnode_t *db, tabl_id_t t_id, row_id_t r_id, size_t count,
                   db_cell_t data[static count]) {
  db_table_t *table = db_table(db, t_id);
  ...
  memcpy(&table->cells[r_id * table->col_count], data,
         sizeof(db_cell_t) * count);
...
string_id_t db_add_string(vnode_t *db, size_t len, char data[static len]) {
  ...
  if (database->string_count >= database->string_capacity) {
    size_t capacity =
        database->string_capacity ? database->string_capacity * 2 : 16;

    db_str_t *strings =
        db_realloc(database->strings, sizeof(db_str_t) * capacity);
...
bool db_get_string(vnode_t *db, string_id_t id, size_t len,

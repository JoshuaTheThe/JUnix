...
#define panic(Code)                                                            \
  panic_impl(__FILE__, __LINE__, Code, #Code, PANIC_CLASS_SUPERVISOR)
#define not_optional(e)                                                        \
  do {                                                                         \
    if ((e) == NULL)                                                           \
      panic(PANIC_NULL_POINTER_DEREFERENCE);                                   \
  } while (0)
#define assert(e)                                                              \
  do {                                                                         \
    if (!(e))                                                                  \
      panic(PANIC_FAILED_ASSERT);                                              \
  } while (0)
...
typedef enum {
  ...
  PANIC_CLASS_USERSPACE,  // Kernel bug triggered by user process (e.g. bad
                          // address)
} panic_class_t;

void panic_impl(const char *const File, long Line, panic_code_t Code,
                const char *const CodeAsStr, panic_class_t Class);

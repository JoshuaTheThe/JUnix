...
static db_type_t table_applications_geometry[] = {db_text, db_text, db_integer,
                                                  db_integer};
static db_type_t table_policies_geometry[] = {db_integer, db_text, db_integer,
                                              db_integer};
...
bool db_init(void) {
  ...
  if (!db_set_table_geometry(system_database, users, l(table_users_geometry),
                             table_users_geometry))
    return false;
  ...
  if (!db_set_table_geometry(system_database, applications,
                             l(table_applications_geometry),
                             table_applications_geometry))
    return false;
  ...
  if (!db_set_table_geometry(system_database, policies,
                             l(table_policies_geometry),
                             table_policies_geometry))
    return false;
  return true;
}

  ...
  for (size_t row = 0; row < font->char_height; row++) {
    ...
    for (size_t col = 0; col < font->char_width; col++) {
      ...
      if (bitmap_row & (1 << (7 - col))) {
        uint32_t *pixel = (uint32_t *)(fb->base + ((y + row) * fb->p) +
...
void fb_draw_rect(fb_t *fb, size_t x, size_t y, size_t w, size_t h,
  ...
  for (size_t i = 0; i < w; i++) {
    if (x + i >= 0 && x + i < fb->w) {
      if (y >= 0 && y < fb->h) {
        uint32_t *pixel =
            (uint32_t *)(fb->base + (y * fb->p) + ((x + i) * bytes_per_pixel));
        *pixel = color;
      }
      if (y + h - 1 >= 0 && y + h - 1 < fb->h) {
        uint32_t *pixel = (uint32_t *)(fb->base + ((y + h - 1) * fb->p) +
                                       ((x + i) * bytes_per_pixel));
        *pixel = color;
      }
    }
  ...
  for (size_t i = 0; i < h; i++) {
    if (y + i >= 0 && y + i < fb->h) {
      if (x >= 0 && x < fb->w) {
        uint32_t *pixel =
            (uint32_t *)(fb->base + ((y + i) * fb->p) + (x * bytes_per_pixel));
        *pixel = color;
      }
      if (x + w - 1 >= 0 && x + w - 1 < fb->w) {
        uint32_t *pixel = (uint32_t *)(fb->base + ((y + i) * fb->p) +
...
void fb_draw_pixel(fb_t *fb, size_t x, size_t y, uint32_t color) {
  ...
  uint8_t bytes_per_pixel = fb->bpp / 8;
  uint32_t *pixel =
      (uint32_t *)(fb->base + (y * fb->p) + (x * bytes_per_pixel));
  *pixel = color;
}

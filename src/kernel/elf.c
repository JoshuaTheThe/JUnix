 ...
 */
static int elf_load_segment(file_t *file, address_space_t *space,
                            Elf_Phdr *ph) {
  Elf_Addr start = PAGE_ALIGN_DOWN(ph->p_vaddr);
  Elf_Addr end = PAGE_ALIGN_UP(ph->p_vaddr + ph->p_memsz);

  /*
   * Allocate virtual memory
   */
  for (Elf_Addr addr = start; addr < end; addr += PAGE_SIZE) {
    void *page = pmm_alloc();
    if (!page)
      return -1;
    uintptr_t phys = virt_to_phys(paging_get_address_space(), page);
    paging_map(space, addr, phys,
  ...
  while (file_left) {
    Elf_Addr page = PAGE_ALIGN_DOWN(virt);
    Elf_Addr offset = virt & (PAGE_SIZE - 1);
    Elf_Addr amount = PAGE_SIZE - offset;
    if (amount > file_left)
      amount = file_left;
   ...
   * Clear BSS
   */
  Elf_Addr zero_start = ph->p_vaddr + ph->p_filesz;
  Elf_Addr zero_end = ph->p_vaddr + ph->p_memsz;
...
int elf_load(file_t *file, address_space_t *space, Elf_Addr *entry) {
  ...
  for (size_t i = 0; i < eh.e_phnum; i++) {
    ...
    if (elf_load_segment(file, space, &ph)) {
      return -1;
    }
  }

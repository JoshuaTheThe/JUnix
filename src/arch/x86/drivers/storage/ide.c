...
void IDEReadBuffer(uint8_t channel, uint8_t reg, uint64_t buffer,
                   uint64_t quads) {
  if (reg > 0x07 && reg < 0x0C)
    IDEWrite(channel, ATA_REG_CONTROL, 0x80 | IDEState.Channels[channel].nIEN);
  if (reg < 0x08)
...
void IDEInitialise(void) {
  if (!IDEState.Dev)
    return;
  int count = 0;
  IDEState.Channels[ATA_PRIMARY].base =
      (IDEState.Dev->bar[0] & 0xFFFFFFFC) + 0x1F0 * (!IDEState.Dev->bar[0]);
  IDEState.Channels[ATA_PRIMARY].ctrl =
      (IDEState.Dev->bar[1] & 0xFFFFFFFC) + 0x3F6 * (!IDEState.Dev->bar[1]);
  IDEState.Channels[ATA_SECONDARY].base =
      (IDEState.Dev->bar[2] & 0xFFFFFFFC) + 0x170 * (!IDEState.Dev->bar[2]);
  IDEState.Channels[ATA_SECONDARY].ctrl =
      (IDEState.Dev->bar[3] & 0xFFFFFFFC) + 0x376 * (!IDEState.Dev->bar[3]);
  IDEState.Channels[ATA_PRIMARY].bmide =
      (IDEState.Dev->bar[4] & 0xFFFFFFFC) + 0; // Bus Master IDE
  IDEState.Channels[ATA_SECONDARY].bmide =
      (IDEState.Dev->bar[4] & 0xFFFFFFFC) + 8; // Bus Master IDE
  ...
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++) {
      ...
      IDEState.IDEDev[count].Signature =
          *((unsigned short *)(&IDEState.Buff[ATA_IDENT_DEVICETYPE]));
      IDEState.IDEDev[count].Capabilities =
          *((unsigned short *)(&IDEState.Buff[ATA_IDENT_CAPABILITIES]));
      IDEState.IDEDev[count].CommandSets =
          *((uint32_t *)(&IDEState.Buff[ATA_IDENT_COMMANDSETS]));

      if (IDEState.IDEDev[count].CommandSets & (1 << 26))
        IDEState.IDEDev[count].Size =
            *((uint32_t *)(&IDEState.Buff[ATA_IDENT_MAX_LBA_EXT]));
      else
        IDEState.IDEDev[count].Size =
            *((uint32_t *)(&IDEState.Buff[ATA_IDENT_MAX_LBA]));

      for (int k = 0; k < 40; k += 2) {
        IDEState.IDEDev[count].Model[k] =
            IDEState.Buff[ATA_IDENT_MODEL + k + 1];
        IDEState.IDEDev[count].Model[k + 1] =
            IDEState.Buff[ATA_IDENT_MODEL + k];
      }
  ...
  for (int i = 0; i < 4; i++)
    if (IDEState.IDEDev[i].Reserved == 1) {
      LOG(" [ide] Found %s Drive %d - %s\r\n",
          ...
          IDEState.IDEDev[i].Size, IDEState.IDEDev[i].Model);
    }
}

unsigned char IDEAccess(unsigned char direction, unsigned char drive,
  ...
  unsigned int slavebit =
      IDEState.IDEDev[drive].Drive; // Read the Drive [Master/Slave]
  unsigned int bus = IDEState.Channels[channel]
                         .base; // Bus Base, like 0x1F0 which is also data port.
  unsigned int words = 256;
  unsigned short cyl, i;
  unsigned char head, sect, err;
  IDEWrite(channel, ATA_REG_CONTROL,
           IDEState.Channels[channel].nIEN = (IDEState.Invoked = 0x0) + 0x02);
  if (lba >= 0x10000000) {
    lba_mode = 2;
  ...
  if (dma)
    ...
    IDEWrite(channel, ATA_REG_COMMAND,
...
unsigned char IDEAtaPIRead(unsigned char drive, unsigned int lba,
  ...
  int i;
  IDEWrite(channel, ATA_REG_CONTROL,
           IDEState.Channels[channel].nIEN = IDEState.Invoked = 0x0);
  ...
  for (i = 0; i < numsects; i++) {
    ...
    __asm volatile("pushw %es");
    __asm volatile("mov %%ax, %%es" ::"a"(selector));
    __asm volatile("rep insw" ::"c"(words), "d"(bus),
                   "D"(edi)); // Receive Data.
    __asm volatile("popw %es");
    edi += (words * 2);
  }
...
void IDEReadSectors(unsigned char drive, unsigned char numsects,
  ...
  if (drive > 3 || IDEState.IDEDev[drive].Reserved == 0)
    package[0] = 0x1;
  else if (((lba + numsects) > IDEState.IDEDev[drive].Size) &&
           (IDEState.IDEDev[drive].Type == IDE_ATA))
    package[0] = 0x2;
  else {
    unsigned char err = 0;
...
void IDEFind(size_t Index) {
  ...
  if (!IDEState.Dev || IDEState.Dev->class_id != 0x01 ||
      IDEState.Dev->subclass_id != 0x01) {

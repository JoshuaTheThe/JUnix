...
bool address_is_user(void *p) {
  ...
  if (current_proc->space != &kernel_address_space && p >= (void *)0xbffff000)
    return false;
  return paging_validate_address(current_proc->space,
                                 (void *)((uintptr_t)p & ~0xFFF));
}
...
size_t user_strnlen(char *p, size_t n) {
  size_t i;
  for (i = 0; address_is_user(&p[i]) && p[i] && n > 0; ++i, --n)
    ;

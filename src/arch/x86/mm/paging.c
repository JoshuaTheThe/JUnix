...
address_space_t *paging_get_address_space(void) { return __space; }

uintptr_t virt_to_phys(address_space_t *as, void *ptr) {
  if ((uintptr_t)ptr >= 0xC0000000 && (uintptr_t)ptr < 0xC1000000 &&
      !paging_active) {
...
void *phys_to_virt(address_space_t *as, uintptr_t addr) {
  if (addr >= 0x200000 && addr < 0x210000 && !paging_active) {
...
void paging_switch(address_space_t *as) {
  ...
  __asm volatile("mov %0, %%cr3" : : "r"(phys) : "memory");
}
...
void paging_disable(void) {
  ...
  __asm volatile("mov %0, %%cr0" : : "r"(cr0) : "memory");
}
...
void paging_map(address_space_t *as, uintptr_t virt, uintptr_t phys,
                uint32_t flags) {
  not_optional(as);
  not_optional(as->pd);
  ...
  if (new_pt) {
    ...
    as->pd->entries[dir] = pt_phys | PAGE_PRESENT | PAGE_WRITE;

    mapping_t *m = &as->items[as->count++];
  ...
  pt->entries[tbl] = (phys & ~0xFFF) | flags | PAGE_PRESENT;
...
void paging_init(void) {
  ...
  mapping_t *m = &kernel_address_space.items[kernel_address_space.count++];
  m->phys =
      virt_to_phys(&kernel_address_space, kernel_address_space.pd) & ~0xFFF;
  ...
  for (uintptr_t addr = 0x200000; addr < 0x1000000; addr += PAGE_SIZE) {
    paging_map(&kernel_address_space, addr + (0xC0000000 - 0x200000), addr,
               PAGE_WRITE);
  }

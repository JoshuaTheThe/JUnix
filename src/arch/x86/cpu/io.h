...
__attribute__((no_caller_saved_registers)) static inline uint8_t
irqinb(uint16_t port) {
  uint8_t value;
  __asm volatile("inb %1, %0" : "=a"(value) : "Nd"(port));
  return value;
}

__attribute__((no_caller_saved_registers)) static inline void
irqoutb(uint16_t port, uint8_t value) {
...
static inline void outsw(uint16_t port, const void *addr, unsigned long n) {
  __asm volatile("cld; rep; outsw" : "+S"(addr), "+c"(n) : "d"(port));
}

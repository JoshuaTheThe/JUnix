...
static void gdt_set(uint32_t num, uint32_t base, uint32_t limit, uint8_t access,
                    uint8_t gran) {
...
static void gdt_tss_set(int num, uint32_t base, uint32_t limit, uint8_t access,
                        uint8_t gran) {
  gdt[num].base_low = (base & 0xFFFF);
  gdt[num].base_middle = (base >> 16) & 0xFF;
  gdt[num].base_high = (base >> 24) & 0xFF;
...
void gdt_init(void) {
  ...
  __asm volatile("ljmp $0x08, $reload_segments \n\t"
                 "reload_segments: \n\t"
                 "mov $0x10, %%ax \n\t"
                 "mov %%ax, %%ds \n\t"
                 "mov %%ax, %%es \n\t"

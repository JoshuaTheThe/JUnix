...
public:
  ...
  BasicFile(const char *path) {
    if ((this->fd = ::open((char *)path, flags)) < 0) {
      ::exit(1);
    }
  }
  BasicFile() {}

  int read(void *const data, size_t count) {
    if (has_read)
      return ::read(this->fd, data, count);
    else
      return -1;
  }
  int write(const void *const data, size_t count) {
    if (has_write)
      return ::write(this->fd, data, count);
    else
      return -1;
  }
  int lseek(int off, int whence = SEEK_SET) {
    if (has_seek)
      return ::lseek(this->fd, off, whence);
    else
      return -1;
  }
};
...
PROCEDURAL int init_memory(void) { return 0; }

PROCEDURAL int shutdown(void) { return 0; }
